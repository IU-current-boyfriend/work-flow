...
import { isNil, cloneDeep, unionSort } from "../cpns/tools/utils";
...
function acquireGeneralFormRules(approval = null) {
  ...
  return Object.entries(rules).reduce((r, [k, v]) => {
    if (Object.keys(approvalGeneral).includes(k)) r[k] = v;
    return r;
  }, {});
}
...
function acquireGeneralFormItems(approval = null) {
  ...
  return computed(() =>
    items.filter((item) => Object.keys(approvalGeneral).includes(item.name))
  );
}
